...
When $|x|$ is large, the gradients are very small, almost zero, which can cause **Vanishing Gradient**. Since its outputs have an average value close to $0.5$ rather than zero, during gradient descent, all outputs tend to move in the same direction, which can cause unnecessary oscillations during the optimization.
...
The output range is $(-1,1)$, making it zero-centered unlike the Sigmoid function. However, it may still cause the **Vanishing Gradient** when $|x|$ is very large.
